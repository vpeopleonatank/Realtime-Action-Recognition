...
def get_command_line_arguments():
    ...
    def parse_args():
        ...
        parser.add_argument("-p", "--data_path", required=False,
                            default="data_test/jumping_jack.mp4",
...
layout = 	[
		...
		[sg.Text('Path to input video', size=(40, 1)),
   sg.In(SRC_DATA_PATH,size=(40,1), key='data_path'), sg.FileBrowse()],
		[sg.Text('Data type ("video", "folder", "webcam")', size=(40,)),
   sg.In(SRC_DATA_TYPE,size=(40,1), key='data_type'), sg.FileBrowse()],
		[sg.Text('Optional Path to output video', size=(40, 1)),
   sg.In(args.output_folder,size=(40,1), key='output'), sg.FileSaveAs()],
		[sg.Text('Action recognition model path', size=(40, 1)),
   sg.In(SRC_MODEL_PATH,size=(40,1), key='model_path'), sg.FolderBrowse()],
		[sg.Text('Confidence', size=(40, 1)),
   sg.Slider(range=(0,1),orientation='h', resolution=.1, default_value=.5,
             size=(30,15), key='confidence')],
		[sg.Text('Threshold', size=(40, 1)), sg.Slider(range=(0,1),
                                                 orientation='h',
                                                 resolution=.1,
                                                 default_value=.3, size=(30,15), key='threshold')],
		# [sg.Text('',size=(40, 1)), sg.Checkbox('Use webcam', key='_WEBCAM_')],
		# [sg.Text('',size=(40, 1)), sg.Checkbox('Write to disk', key='_DISK_')],
		[sg.OK(), sg.Cancel()]
			]
...
SRC_DATA_TYPE = values["data_type"]
if SRC_DATA_TYPE in ["video", "folder"]:
    SRC_DATA_PATH = values["data_path"]
SRC_MODEL_PATH = values["model_path"]
...
if __name__ == "__main__":
    ...
    try:
        ...
        while images_loader.has_image():
            ...
            imgbytes = cv2.imencode('.png', img_disp)[1].tobytes()  # ditto
            if not win_started:
                win_started = True
                layout = [
